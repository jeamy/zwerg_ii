...
static double clampDec(double decDeg) {
    if (decDeg > 90.0) return 90.0;
    if (decDeg < -90.0) return -90.0;
    return decDeg;
}

static bool parseRaToDegrees(const QByteArray &value, bool highPrecision, double *outRaDeg) {
...
static bool parseDecToDegrees(const QByteArray &value, bool highPrecision, double *outDecDeg) {
    ...
    double decDeg = sign * (static_cast<double>(deg) + (static_cast<double>(min) / 60.0) + (static_cast<double>(sec) / 3600.0));
...
Lx200Server::Lx200Server(QObject *parent)
    ...
    , m_targetRaDeg(0.0)
    , m_targetDecDeg(0.0)
    , m_haveTargetRa(false)
    , m_haveTargetDec(false)
    , m_latitudeDeg(0.0)
    , m_longitudeDeg(0.0)
    , m_highPrecision(false)
{
    connect(m_server, &QTcpServer::newConnection, this, &Lx200Server::onNewConnection);
}
...
bool Lx200Server::start(quint16 port, const QHostAddress &address) {
    ...
    qDebug() << "[LX200] Listening on" << m_server->serverAddress().toString() << ":" << m_server->serverPort();
    return true;
}
...
bool Lx200Server::isRunning() const {
    return m_server->isListening();
}

quint16 Lx200Server::listeningPort() const {
...
int Lx200Server::clientCount() const {
...
void Lx200Server::onNewConnection() {
    while (m_server->hasPendingConnections()) {
        ...
        connect(sock, &QTcpSocket::readyRead, this, &Lx200Server::onClientReadyRead);
        connect(sock, &QTcpSocket::disconnected, this, &Lx200Server::onClientDisconnected);
        connect(sock, &QTcpSocket::errorOccurred, this, [this, sock](QAbstractSocket::SocketError err) {
            // RemoteHostClosedError is normal when client disconnects - don't treat as error
            if (err != QAbstractSocket::RemoteHostClosedError) {
                qWarning() << "[LX200] Client socket error:" << sock->errorString();
            }
        });

        emit clientConnected(sock->peerAddress(), sock->peerPort());
        qDebug() << "[LX200] Client connected" << sock->peerAddress().toString() << ":" << sock->peerPort();
    }
}
...
void Lx200Server::onClientDisconnected() {
    ...
    qDebug() << "[LX200] Client disconnected" << sock->peerAddress().toString() << ":" << sock->peerPort();
...
void Lx200Server::processBuffer(QTcpSocket *socket) {
    ...
    while (!buffer.isEmpty() && static_cast<unsigned char>(buffer.at(0)) == 0x06) {
        buffer.remove(0, 1);
...
QByteArray Lx200Server::handleCommand(const QByteArray &cmd, const QByteArray &value) {
    // Note: Return strings for LX200 are generally terminated with '#'.
    // Some commands return raw strings without '#', but clients generally accept with '#'.
    if (cmd == ":GR") {
    ...
    if (cmd == ":Q" || cmd == ":Qn" || cmd == ":Qs" || cmd == ":Qe" || cmd == ":Qw") {
        emit stopRequested();
        return QByteArray();
    }
    ...
    if (cmd == ":Gg") {
        // Get longitude DDD*MM# (unsigned)
        double lon = normalizeDegrees360(m_longitudeDeg);
        int ddd = static_cast<int>(std::floor(lon));
        int mm = static_cast<int>(std::round((lon - ddd) * 60.0));
        if (mm >= 60) {
            mm = 0;
            ddd = (ddd + 1) % 360;
        }
    ...
    if (cmd == ":Sg") {
        ...
        m_longitudeDeg = static_cast<double>(ddd) + (static_cast<double>(mm) / 60.0);
        return QByteArray("1");
    }
